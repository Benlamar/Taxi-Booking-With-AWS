...
const Panel = () => {
  ...
  const handleBooking = (e, id) => {
    ...
    if (type !== "") {
      ...
      axios.post(
        // api endpoint to find taxi
        "",
        JSON.stringify(request_booking)
      )
        .then((res) => {
          console.log(res)
          setTaxisPoint(res.data);
  ...
  const handleType = (e, key) => {
    if (
      ...
      e.target.value === "Deluxe" || e.target.value === "Any" ||
  ...
  }, [destinationList, startPoint, destinationPoint, taxisPoint]);
